...
# ----------------------------------------------------------------------------------------------------------------------
def get_gb_ineligible():
    sql = "SELECT CONCAT(per_FirstName,' ',per_LastName) AS Name, family_custom.c7 FROM `person_per`  INNER JOIN `family_fam` ON family_fam.fam_ID = person_per.per_fam_ID INNER JOIN `family_custom` ON family_custom.fam_ID = person_per.per_fam_ID WHERE person_per.per_fmr_ID in (1,2) AND person_per.per_cls_ID <> 4 AND person_per.per_id not in (select r2p_record_id from record2property_r2p where record2property_r2p.r2p_pro_ID = 12) AND family_custom.c5 = 'FALSE' AND family_fam.fam_DateDeactivated is null ORDER BY family_custom.c7"
